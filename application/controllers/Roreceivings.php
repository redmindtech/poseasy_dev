...
class Roreceivings extends Secure_Controller
{
    ...
    {
        $data['voucher_no'] = $this->Ro_receiving->invoice_no_autogenerate();
        // var_dump($data['voucher_no']);
    ...
    public function check_invoice_number($supplier_id)
    {
		echo "hi";
        $result = $this->Ro_receiving->invoice_no_autogenerate($supplier_id);
        $voucher_no = 0;
        if ($result != null) {
            foreach ($result as $row) {
        ...
        if ($voucher_no == null || $result == null) {
            $voucher_no = 0;
        }
        echo $voucher_no;
    }
